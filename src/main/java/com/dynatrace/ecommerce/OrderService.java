...
public class OrderService {
    ...
    // Utility method for safely closing ResultSet and Statement
    private static void closeQuietly(ResultSet rs, PreparedStatement stmt) {
        if (rs != null) {
            try {
                rs.close();
     ...
     * FIXED: Now uses try-with-resources so connections, statements, and result sets are always closed.
    ...
    private List<OrderItem> getOrderItemsWithValidation(int orderId) throws SQLException {
        List<OrderItem> items = new ArrayList<>();
        // Try-with-resources guarantees closure on all paths (including exceptions)
        try (Connection conn = connectionPool.getConnection();
             PreparedStatement stmt = conn.prepareStatement(
                 "SELECT item_id, product_name, quantity, price FROM order_items WHERE order_id = ?")
        ) {
     ...
     * FIXED: Uses try-with-resources so connection is always closed, even on exception. Clean code, robust error handling.
     */
